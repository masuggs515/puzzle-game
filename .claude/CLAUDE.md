...
The Manager spawns a sub-agent when a task is deeply focused in a single domain and benefits from isolation. The Manager handles cross-layer tasks directly without spawning agents unnecessarily.

### When to spawn vs. handle directly

**Handle directly (no spawn):**
- Reading spec files, answering questions
- Small fixes touching one file in a familiar domain
- Git operations (always Manager-only)
- Cross-layer tasks that require coordinating multiple domains

**Spawn a sub-agent:**
- Deep, focused work in a single specialist domain
- Tasks the Manager would otherwise context-switch through multiple specs to complete
- Review and test runs (always use the dedicated agents)

### Sub-Agent Registry

| Agent | File | Spawn when... |
|---|---|---|
| `supabase` | `.claude/agents/supabase.md` | Writing migrations, RLS policies, Edge Functions, or auth config |
| `flutter` | `.claude/agents/flutter.md` | Implementing screens, widgets, game state, or SDK integration |
| `testing` | `.claude/agents/testing.md` | Running tests after any code change or before marking a phase complete |
| `review` | `.claude/agents/review.md` | Before every merge — spec compliance and breaking-change check |
| `puzzle-generation` | `.claude/agents/puzzle-generation.md` | Constraint library, CSP solver, word list pipeline, pre-generation CLI |
| `analytics` | `.claude/agents/analytics.md` | Event schema, AnalyticsService implementation, PII compliance |
| `level-design` | `.claude/agents/level-design.md` | Designing hand-crafted puzzles, boss levels, difficulty curves |

### Sub-agent rules

- Sub-agents do NOT run Git commands — the Manager commits their output
- Sub-agents return a clear result/summary; Manager reviews before committing
- Testing agent runs after every agent session that produces code
- Review agent runs before every PR merge
- Never spawn multiple agents for the same task — sequential is fine, parallel is for genuinely independent work
